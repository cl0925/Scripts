...
首次使用时,  需要手动登录京东网页版签到一次获取Cookie,  如Surge弹出获取成功通知时，即可禁用HTTP request脚本。
脚本为每日9:00执行签到 您可以修改执行时间。
由于Cookie的有效性，如果脚本将来弹出Cookie无效的通知，则需要重复上述步骤。
-----------------------
Quantumult X 说明:
QX版脚本仅适配TF1.0.5 (166)以上 低于此版本请勿使用。

由于QX的限制, 无法像Surge那样进行双脚本联动存入Cookie, 故采用抓取Cookie后用捷径导入脚本, 捷径点此下载
抓包导入演示点此处
-----------------------

目前存在的一些问题:
如接口3提示已签到 为正常现象。
因需要手动登入此网址签到一次, 实测七天左右需要再次登录签到, 后续将完善此接口。

内测用户:
首先感谢内测用户反馈的一些问题, 并且内测过此脚本的同学请尽快替换成以上脚本链接, 私有gist链接将在几天内移除。
